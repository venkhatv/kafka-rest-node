...
Consumer.prototype.join = function(opts, res) {
    return new Promise((resolve, reject) => {
        ...
        else if (opts && opts.format === 'json')
            this.schemaClass = JsonSchema;
...
var ConsumerInstance = Consumers.ConsumerInstance = function(consumer, raw) {
    ...
    this.raw = raw;
    this.raw.base_uri = this.raw.base_uri.replace('http://', 'https://');
    if (!this.id || !this.uri)
 ...
 * @param {Object} topicsObj Array of topic name strings or topic_pattern
...
ConsumerInstance.prototype.subscription = function(topicsObj, options, res) {
    return new Promise((resolve, reject) => {
        ...
        if (!this.client._validateVersionSupport(
                supportedVersions, utils.getPromiseCallback(resolve, reject, res)
            )) return;
        var stream;
        this.client.post(utils.urlJoin(this.getUri(), 'subscription'), topicsObj, null, function(err) {
            if (err) return utils.getPromiseCallback(resolve, reject, res)(err);
            if (this.streams.length) this._shutdownStreams();
...
ConsumerInstance.prototype.cancelSubscription = function(res) {
    return new Promise((resolve, reject) => {
        var supportedVersions = [2];
        if (!this.client._validateVersionSupport(supportedVersions, utils.getPromiseCallback(resolve, reject, res))) return;
        this.client.delete(utils.urlJoin(this.getUri(), 'subscription'), function(err) {
            if (err) return utils.getPromiseCallback(resolve, reject, res)(err);
            this._shutdownStreams();
            utils.getPromiseCallback(resolve, reject, res)(null, true);
        }.bind(this));
    });
}
...
ConsumerInstance.prototype.getAssignment = function(res) {
    return new Promise((resolve, reject) => {
        var supportedVersions = [2];
        if (!this.client._validateVersionSupport(supportedVersions, utils.getPromiseCallback(resolve, reject, res))) return;
...
ConsumerInstance.prototype.position = function(body, position, res) {
    return new Promise((resolve, reject) => {
        ...
        var positionUrl = !position ? utils.urlJoin(this.getUri(), 'positions') : utils.urlJoin(this.getUri(), 'positions', position);
