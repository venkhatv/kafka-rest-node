 ...
 * @param {*} options Object which can options for the stream 
...
ConsumerInstance.prototype.subscription = function(topicsObj, options, res) {
    if ( res === undefined ) {
        const func = this.subscription.bind(this);
        return new Promise(function (resolve, reject) { 
            func(topicsObj, options, function (err, result) {
                err ? reject(err) : resolve(result);
            })
        })
    }
    var supportedVersions = [2];
    var isSupported = this.client._validateVersionSupport(supportedVersions, res);
    var stream;
    if(!isSupported) {
        return;
    }
...
ConsumerInstance.prototype.getSubscriptionDetails = function(res) {
    if ( res === undefined ) {
        const func = this.getSubscriptionDetails.bind(this);
        return new Promise(function (resolve, reject) { 
    ...
    this.client.request(utils.urlJoin(this.getUri(), 'subscription'), res)    
}


ConsumerInstance.prototype.cancelSubscription = function(res) {
    if ( res === undefined ) {
        const func = this.cancelSubscription.bind(this);
        return new Promise(function (resolve, reject) { 
            func(function (err, result) {
                err ? reject(err) : resolve(result);
            })
