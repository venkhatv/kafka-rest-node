...
Topics.prototype.list = function(res) {
    return new Promise((resolve, reject) => {
        this.client.request("/topics", function(err, topics_response) {
            ...
            utils.getPromiseCallback(resolve, reject, res)(null, topics);
        }.bind(this));
    });
}
...
Topics.prototype.get = function(name, res) {
    return new Promise((resolve, reject) => {
        ...
        //Have a dummy catch block, to avoid  Unhandled promise rejections warning
        topic.get(utils.getPromiseCallback(resolve, reject, res)).catch(()=>{});
        return topic;
    });
}
...
Topic.prototype.get = function(res) {
    return new Promise((resolve, reject) => {
        this.client.request(this.getPath(), function(err, topic_raw) {
            if (err) return utils.getPromiseCallback(resolve, reject, res)(err);
            this.raw = topic_raw;
            utils.getPromiseCallback(resolve, reject, res)(null, this);
        }.bind(this));
    });
}
    ...
    return new Promise((resolve, reject) => {
        this.client.post(this.getPath(), request, schemas[0] || schemas[1], function(err, produce_response) {
            if (err) {
                ...
                utils.getPromiseCallback(resolve, reject, res)(null, produce_response);
            }
        }.bind(this));
    });
};
