...
var Client = module.exports = function(config) {
    ...
    if (config.requestOptions) this.config.requestOptions = config.requestOptions;
...
Client.prototype.SUPPORTED_VERSIONS = [1, 2];
...
Client.prototype.request = function(opts, res) {
    ...
    if(this.config.requestOptions) Object.assign(reqOpts, this.config.requestOptions);
...
//Use this, when body is sent in Get request
Client.prototype.get = function(path, body, schema, res) {
...
/**
 * Validates whether the client supports the api/method calling this function and throws error if not supported
 * @param {Array} supportedVersions Array fof version numbers that is supported by the calling method
 * @param {*} res
 */
Client.prototype._validateVersionSupport = function(supportedVersions, res) {
    var clientVersion = this.config.version;
    if (supportedVersions.indexOf(clientVersion) === -1) {
        if(res) res(new Error('Method not supported for version ' + clientVersion));
        return false;
    }
    return true;
}
