...
kafka
    ...
    .join(consumerConfig, async function(err, consumer_instance) {
        if (err)
        ...
        var stream;
        try {
            stream = await consumer_instance.subscription(
                subscribedTopics,
                subscriptionOptions
            );
            ...
            stream.on("data", function(msgs) {
                for (var i = 0; i < msgs.length; i++) {
                    if (format == "binary") {
                        // Messages keys (if available) and values are decoded
                        // from base64 into Buffers. You'll need to decode based
                        // on whatever serialization format you used. By default
                        // here, we just try to decode to text.
